...
import { selectQuestion } from "../store/Exam/thunk";

import { Button } from "antd";
const FooterExam = (props) => {
  const { currentTime, totalTime, numberOfQuestion, questionSelected } = props;

  const dispatch = useDispatch();
  const handleNextQue = () => {
    if (questionSelected < numberOfQuestion) {
      dispatch(selectQuestion(questionSelected + 1));
    } else {
      return;
    }
  };
    ...
    <section className="flex items-center justify-between w-[480px]">
      <div className="w-4/12 h-full flex items-center justify-between gap-2 ">
        <Button
          ...
          disabled={
            questionSelected && questionSelected > 1 ? false : true
          }
